...
Data and tokenizer are stored in cache/.
"""
...
def prepare_with_cmf(num_shards=10, download_workers=8):
    """Prepare data and tokenizer with CMF tracking."""
    from cmflib.cmf import Cmf

    graph_env = os.getenv("NEO4J", "False")
    graph = True if graph_env == "True" or graph_env == "TRUE" else False
    metawriter = Cmf(filepath="mlmd", pipeline_name="autoresearch", graph=graph)
    _ = metawriter.create_context(
        pipeline_stage="Prepare", custom_properties={"stage": "data_prep"}
    )
    ...
    download_data(num_shards, download_workers=download_workers)
    _ = metawriter.log_dataset(DATA_DIR, "output")
    print()

    # Step 2: Train tokenizer
    train_tokenizer()
    _ = metawriter.log_dataset(
        TOKENIZER_DIR, "output", custom_properties={"vocab_size": VOCAB_SIZE}
    )
    print()
...
if __name__ == "__main__":
    ...
    if args.with_cmf:
        prepare_with_cmf(num_shards, args.download_workers)
        # Create marker so train.py automatically uses CMF too
        with open(".cmf_enabled", "w") as f:
            f.write("CMF enabled for this run\n")
    else:
        print("Running without CMF instrumentation")
        print(f"Cache directory: {CACHE_DIR}")
        print()
